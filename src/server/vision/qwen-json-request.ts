...
export async function requestQwenJson<T>(options: { apiKey: string; prompt: string; payload: unknown; requestType: AIRequestType; validate: (value: unknown) => T; onProgress?: (event: VisionAnalysisProgressEvent) => void; signal?: AbortSignal; fetcher?: typeof fetch; sleep?: (ms: number) => Promise<void>; diagnostics?: QwenRequestDiagnostics; timeoutMs?: number; maxAttempts?: number }) {
  const fetcher = options.fetcher ?? fetch; const sleep = options.sleep ?? ((ms: number) => new Promise<void>((resolve) => setTimeout(resolve, ms))); const timeoutMs = options.timeoutMs ?? QWEN_REQUEST_TIMEOUT_MS; const maxAttempts = options.maxAttempts ?? QWEN_MAX_ATTEMPTS; let repair = false; let lastError: unknown;
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    options.onProgress?.({ state: "attempt", attempt, maxAttempts, model: QWEN_VISION_MODEL });
    try {
      return await globalAIRequestScheduler.schedule(options.requestType, "high", async () => {
        ...
        } catch (error) { if (controller.signal.aborted) throw new AiProviderError("AI_TIMEOUT", `Qwen timed out after ${timeoutMs}ms`, true); throw error; }
      ...
      if (providerError.code === "PROVIDER_ERROR" && attempt < maxAttempts) { repair = true; continue; }
      if (!providerError.retryable || attempt === maxAttempts) throw providerError;
      options.onProgress?.({ state: "retrying", attempt: attempt + 1, maxAttempts, model: QWEN_VISION_MODEL, category: providerError.code, delayMs: 1500 }); await sleep(1500);
    }
  }
  throw lastError;
}
